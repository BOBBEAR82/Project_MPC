 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t i=0; i<N; i++) 
	{
		...
		// Since a very big weight applied on "delta*v" (line 79), this is to keep the vehicle from running 
		// in very low speed, even stop, during start and big turns.
		if (vars[v_start+i] < 15) fg[0] += 30 * CppAD::pow(vars[v_start + i] - ref_v, 2);
    }
    ...
    for (size_t i = 0; i < N - 1; i++) 
	{
		...
		// Very important to achieve stable run by "lower the speed during turns"
		fg[0] += 8000 * CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
    }
    ...
    for (size_t i = 1; i < N; i++) {
		...
		// Calculate y in the trajectory.
		AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * CppAD::pow(x0, 2) + coeffs[3] * CppAD::pow(x0, 3);
		
		// Calculate ideal vehicle heading in the trajectory.
